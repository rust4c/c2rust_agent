...
if __name__ == "__main__":
    ...
    subparsers = parser.add_subparsers(
        dest='command', help='可用命令', required=True)
    ...
    analyze_relations_parser = subparsers.add_parser(
        'analyze-relations', help='分析C项目调用关系并保存到数据库')
    ...
    if ARGS.command == "analyze":
        ...
        lsp_services.check_function_and_class_name(
            project_path, detailed=False)
        ...
        project_path = ARGS.input_dir
        project_name = ARGS.project_name or os.path.basename(
        ...
        try:
            ...
            print(f"使用以下命令查看结果:")
            print(
                f"  python run.py relation-query --db {db_path} --command report --project {project_name}")
        ...
        try:
            if ARGS.query_type == 'list-projects':
                ...
                definitions = query_tool.find_function_definition(
                    ARGS.project, ARGS.target)
                calls = query_tool.find_function_calls(
                    ARGS.project, ARGS.target)
                ...
                if definitions:
                    ...
                    for defn in definitions[:5]:  # 只显示前5个
                        ...
                        print(
                            f"    {defn['function_name']} in {file_name}:{defn['line_number']}")

            elif ARGS.query_type == 'call-chain' and ARGS.project and ARGS.target:
                chain = query_tool.get_function_call_chain(
                    ARGS.project, ARGS.target)
                ...
                results = query_tool.search_function_usage(
                    ARGS.project, ARGS.keyword)
                ...
                usage = get_function_usage_summary(
                    query_tool, ARGS.project, ARGS.target)
                ...
                if usage.get('called_by'):
                    ...
                    for call in usage['called_by'][:5]:
                        file_name = Path(call['caller_file']).name if call.get(
                            'caller_file') else 'unknown'
                        caller_func = call.get('caller_function') or 'unknown'
                        print(
                            f"    {caller_func} in {file_name}:{call.get('caller_line', '?')}")
