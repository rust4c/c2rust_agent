...
pub async fn check_all_ai_services() -> Result<AIConnectionStatus> {
    ...
    let mut deepseek_connected = false;
    let mut ollama_connected = false;
    let mut openai_connected = false;
    let mut xai_connected = false;
    ...
    deepseek_connected = deepseek_result.is_ok();
    ollama_connected = ollama_result.is_ok();
    openai_connected = openai_result.is_ok();
    xai_connected = xai_result.is_ok();
    
    // 确定连接状态
    let connected_count = [deepseek_connected, ollama_connected, openai_connected, xai_connected]
        .iter()
        .filter(|&&s| s)
        .count();
    
    let connection_status = match connected_count {
        4 => AIConnectionStatus::AllConnected,
        1..=3 => AIConnectionStatus::PartiallyConnected,
        0 => AIConnectionStatus::AllDisconnected,
...
pub async fn get_detailed_ai_status() -> Result<String> {
    ...
    let status_info = format!(
        "当前配置的 AI 服务: {}\n\
         ...
         XAI 状态: {}",
        config.provider,
        if deepseek_result.is_ok() { "连接正常" } else { "连接失败" },
        if ollama_result.is_ok() { "连接正常" } else { "连接失败" },
        if openai_result.is_ok() { "连接正常" } else { "连接失败" },
        if xai_result.is_ok() { "连接正常" } else { "连接失败" }
    );
