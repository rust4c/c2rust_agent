...
pub async fn dbdata_init(db_manager: DatabaseManager) -> Result<DatabaseConnectionStatus> {
    ...
    match connection_status {
        DatabaseConnectionStatus::BothConnected => info!("数据库连接正常: SQLite 和 Qdrant 都已连接"),
        DatabaseConnectionStatus::OnlySqliteConnected => warn!("数据库连接部分正常: 只有 SQLite 已连接"),
        DatabaseConnectionStatus::OnlyQdrantConnected => warn!("数据库连接部分正常: 只有 Qdrant 已连接"),
        DatabaseConnectionStatus::BothDisconnected => warn!("数据库连接失败: SQLite 和 Qdrant 都未连接"),
